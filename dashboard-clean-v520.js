...
function money(v){return Number(v||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL',minimumFractionDigits:0,maximumFractionDigits:0})}
function recordValue(row){return typeof window.getPreSaleFinalValue==='function'?Number(window.getPreSaleFinalValue(row)||0):Number(row?.valorFinal||0)}
function revenueBy(rows,key){const out={};for(const row of rows){const label=key(row),value=recordValue(row);if(label&&value>0)out[label]=(out[label]||0)+value}return out}
function revenueBars(obj,rows){const entries=Object.entries(obj).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'pt-BR'));if(!entries.length)return'<div class="dash-clean-empty">Sem valores cadastrados no período.</div>';const max=Math.max(...entries.map(x=>x[1]),1);return entries.map(([label,value])=>{const own=rows.filter(x=>(x.vendedor||'Não informado')===label),products={};for(const row of own){const key=row.produto||'Não informado';if(!products[key])products[key]={count:0,value:0};products[key].count++;products[key].value+=recordValue(row)}const detail=Object.entries(products).sort((a,b)=>b[1].value-a[1].value).map(([name,x])=>`<div><span>${esc(name)}</span><small>${x.count} pré-venda(s)</small><strong>${esc(money(x.value))}</strong></div>`).join('');return`<div class="dash-consultant-revenue"><button type="button" class="dash-revenue-row" aria-expanded="false"><span>${esc(label)}</span><i><b style="width:${Math.round(value/max*100)}%"></b></i><strong>${esc(money(value))}</strong><em>⌄</em></button><div class="dash-consultant-detail" hidden>${detail}</div></div>`}).join('')}
function ensure(){
...
function renderOverview(rows){
  const eco=rows.filter(x=>x.wearable).length,pre=rows.filter(x=>String(x.preregistro).toUpperCase()==='SIM').length,trade=rows.filter(x=>String(x.trocafone).toUpperCase()==='SIM').length,totalRevenue=rows.reduce((sum,x)=>sum+recordValue(x),0),priced=rows.filter(x=>recordValue(x)>0).length;
  return`<div class="dash-clean-kpis">${kpi('Pré-vendas',rows.length,'no período')}${kpi('Faturamento total',money(totalRevenue),`${priced} com valor-base`)}${kpi('Pré-registro',pre)}${kpi('Trade-in',trade)}${kpi('Taxa de ecossistema',rows.length?Math.round(eco/rows.length*100)+'%':'0%')}</div><div class="dash-clean-panel dash-clean-main"><div class="dash-clean-heading"><div><span>Desempenho</span><h3>Ranking por consultor</h3></div><small>${rows.length} pré-venda(s)</small></div>${bars(group(rows,x=>x.vendedor||'Não informado'))}</div><div class="dash-clean-panel dash-revenue-panel"><div class="dash-clean-heading"><div><span>Receita</span><h3>Faturamento por consultor</h3></div><small>Clique para detalhar por produto</small></div>${revenueBars(revenueBy(rows,x=>x.vendedor||'Não informado'),rows)}</div>`;
}
...
function includeCatalogOptions(counts,items){const out={...counts};for(const item of items||[])if(item&&!Object.prototype.hasOwnProperty.call(out,item))out[item]=0;return out}
function productAccordion(rows){
  ...
  return products.map(([product,total])=>{
    const own=rows.filter(x=>x.produto===product),catalogProduct=(typeof getCatalog==='function'?getCatalog():[]).find(x=>x.name===product),colors=includeCatalogOptions(group(own,x=>x.cor||'Não informada'),catalogProduct?.colors),memories=includeCatalogOptions(group(own,x=>/^não se aplica$/i.test(String(x.capacidade||''))?'':x.capacidade||'Não informada'),catalogProduct?.capacities),hasMemory=(catalogProduct?.capacities||[]).length>0||Object.keys(memories).length>0;
...
function renderClean(){
  ...
  content.querySelectorAll('.dash-revenue-row').forEach(button=>button.onclick=()=>{const detail=button.nextElementSibling,open=button.getAttribute('aria-expanded')==='true';button.setAttribute('aria-expanded',String(!open));detail.hidden=open});
}

const base=window.renderDashboard;
window.renderDashboard=function(){if(typeof base==='function')base();renderClean()};
window.getDashboardRows=()=>data();
window.getDashboardFilterState=()=>({...filters(),products:[...selectedProducts]});
ensure();renderClean();
...
.dash-clean-kpis{display:grid;grid-template-columns:repeat(auto-fit,minmax(155px,1fr));gap:12px;margin-bottom:14px}.dash-clean-kpis-two{grid-template-columns:repeat(2,1fr)}.dash-clean-kpi{padding:16px;background:#fff;border:1px solid #e4e7ec;border-radius:14px}.dash-clean-kpi span{display:block;font-size:12px;color:#667085}.dash-clean-kpi strong{display:block;margin-top:6px;font-size:25px;color:#101828}.dash-clean-kpi small{color:#98a2b3;font-size:11px}
