(()=>{
'use strict';
const EXPECTED='8.2.2',ONCE_KEY='preVendaV822Recovery';
...
async function clearRuntimeCaches(){if('serviceWorker'in navigator){const regs=await navigator.serviceWorker.getRegistrations();await Promise.all(regs.map(r=>r.update().catch(()=>null)))}if('caches'in window){const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('pre-venda-samsung-')&&!k.includes('v8-2-2')).map(k=>caches.delete(k)))}}
async function verify(){for(let i=0;i<3;i++){try{const [v,h]=await Promise.all([fetchJson('./version.json'),fetchJson('./health.json')]);const ok=v.version===EXPECTED&&v.channel==='stable'&&v.published===true&&h.version===EXPECTED;document.documentElement.dataset.deployHealth=ok?'pass':'mismatch';window.V822_DEPLOY_HEALTH={ok,version:v.version,health:h.version,checkedAt:new Date().toISOString()};if(ok){sessionStorage.removeItem(ONCE_KEY);return true}break}catch(e){window.V822_DEPLOY_HEALTH={ok:false,error:e.message,checkedAt:new Date().toISOString()};await sleep(500)}}return false}
...
window.V741DeployGuard=window.V80DeployGuard=window.V81DeployGuard=window.V82DeployGuard=window.V821DeployGuard=window.V822DeployGuard={version:EXPECTED,verify,recover,clearRuntimeCaches};
