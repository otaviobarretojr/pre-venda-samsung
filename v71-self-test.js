...
async function run(){try{
 ...
 document.documentElement.dataset.v74Smoke=report.ok?'pass':'fail';window.V74_SELF_TEST=report;window.dispatchEvent(new CustomEvent('pre-sale:v74-self-test',{detail:report}));console[report.ok?'info':'error']('[V7.4 self-test]',report);return report}
window.runV74SelfTest=run;window.runV73SelfTest=run;window.runV72SelfTest=run;window.runV71SelfTest=run;setTimeout(run,1800);window.addEventListener('load',()=>setTimeout(run,900),{once:true});
})();
