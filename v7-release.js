...
'use strict';
const RELEASE='7.0.2';
const CACHE_EPOCH='v7-0-2-budget-catalog-refresh';
...
const loadValidation=async()=>{if(window.__V7_VALIDATION_LOADED__)return;window.__V7_VALIDATION_LOADED__=true;try{await loadScript('./budget-catalog-refresh-v702.js?release='+encodeURIComponent(RELEASE),'budget-refresh');await loadScript('./pre-sale-watches-v7.js?release='+encodeURIComponent(RELEASE),'watches');await loadScript('./v7-smoke-test.js?release='+encodeURIComponent(RELEASE),'smoke')}catch(e){document.documentElement.dataset.v7Smoke='load-error';console.error('[V7] validation module failed to load',e)}};
applyIdentity();const root=document.body||document.documentElement;if(root)new MutationObserver(scheduleIdentity).observe(root,{childList:true,subtree:true});window.addEventListener('load',()=>{scheduleIdentity();setTimeout(loadValidation,350)},{once:true});setTimeout(()=>{applyIdentity();window.preSaleReleaseBoot?.()},0);setTimeout(applyIdentity,250);setTimeout(applyIdentity,1000);if(document.readyState==='complete')setTimeout(loadValidation,350);console.info('[Pré-Venda Samsung] release',RELEASE,'loaded');
})();