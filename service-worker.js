...
self.addEventListener('fetch',event=>{
 ...
 event.respondWith((immutable?caches.match(req).then(hit=>hit||fetch(req)):fetch(req,{cache:'no-store'})).then(response=>{if(response&&response.ok&&url.origin===self.location.origin){const clone=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(req,clone))}return response}).catch(()=>caches.match(req)))
});
