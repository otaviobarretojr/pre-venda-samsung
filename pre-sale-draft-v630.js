...
window.preSaleHasUnsavedDraft=()=>meaningful();
...
const baseSave=window.saveCurrent;if(typeof baseSave==='function'){window.saveCurrent=async function(){const result=await baseSave.apply(this,arguments);if(result)clear();else persist();return result};const button=$id('salvarBtn');if(button)button.onclick=window.saveCurrent}
const baseNew=window.startNewSale;if(typeof baseNew==='function')window.startNewSale=function(){clear();return baseNew.apply(this,arguments)};
window.preSaleDraft={snapshot,persist,restore,clear,meaningful};
})();